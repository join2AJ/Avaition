...
export const SCHEMA: Tbl[] = [
  ...
  },
  {
    ...
    cols: [
      ...
      { name: "individual_id", type: "varchar(16)", key: "FK", ref: "individuals.id", nullable: true, note: "MAN — the AEP holder this pass belongs to" },
  ...
  },
  {
    ...
    cols: [
      ...
      { name: "individual_id", type: "varchar(16)", key: "FK", ref: "individuals.id", note: "the carrier — MUST be a valid AEP holder (issued, in-validity, zone-covering, not stop-listed)" },
...
const INSIGHT: Record<string, string> = {
  ...
  applications: "The workflow core. Correlate to entities (sponsor), individuals (the AEP holder, MAN), contracts, pillars, pass_types, committees, passes, step_logs, application_zones. A MAN application in-validity = a valid AEP holder.",
  ...
  material_movements: "Every in / consumed / out event. individual_id (carrier) MUST be a valid AEP holder — join individuals → applications (issued MAN AEP covering the gate's zone). Per material_id: SUM(in) − SUM(consumed) − SUM(out) = qty still inside; join gates, passes, users (CISF) for full custody.",
